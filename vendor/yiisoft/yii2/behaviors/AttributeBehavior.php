 ...
 * Then configure the [[value]] property with a PHP callable whose return value will be used to assign to the current attribute(s).
 * For example,
 *
 * ```php
 ...
 * Because attribute values will be set automatically by this behavior, they are usually not user input and should therefore not be validated,
 * i.e. they should not appear in the [[\yii\base\Model::rules()|rules()]] method of the model.
 *
...
class AttributeBehavior extends Behavior
{
     ...
     * The array keys are the ActiveRecord events upon which the attributes are to be updated, and the array values are the corresponding attribute(s) to be updated.
     ...
     * 如果是前者，函数的返回值将被分配给属性。
     *
     * @var mixed the value that will be assigned to the current attributes.
     * This can be an anonymous function, callable in array format (e.g. `[$this, 'methodName']`),
     * an [[Expression]] object representing a DB expression (e.g. `new Expression('NOW()')`), scalar, string or an arbitrary value.
     * If the former, the return value of the function will be assigned to the attributes.
     ...
     * @var boolean whether to skip this behavior when the `$owner` has not been modified
     * @since 2.0.8
     *
