...
class BaseUrl
{
     ...
     * while a relative route has none (e.g. `site/index` or `index`).
     *
    ...
    public static function to($url = '', $scheme = false)
    {
        ...
        // 若$url 中不包含冒号 或者 冒号前的字符不是只包含英文字母，则直接将当前主机地址拼接到$url前。
        // ctype_alpha() 检查字符串是否只包含英文字母
    ...
    public static function home($scheme = false)
    {
        ...
        if ($scheme) {
            ...
            $url = static::getUrlManager()->getHostInfo() . $url;
            // 如果$scheme是字符串，则使用其替换掉原URL的协议部分
            if (is_string($scheme) && ($pos = strpos($url, '://')) !== false) {
                $url = $scheme . substr($url, $pos);
            }
        }
     ...
     * will be removed from the existing GET parameters;
     * all other parameters specified in `$params` will be merged with the existing GET parameters.
     * For example,
     *
     * ```php
