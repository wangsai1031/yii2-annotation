...
class ActiveRecord extends BaseActiveRecord
{
    ...
    public function loadDefaultValues($skipIfSet = true)
    {
        ...
        foreach (static::getTableSchema()->columns as $column) {
            // 若当前列设置了默认值 && (不保留已经存在的值 || 当前列的值为 null), 则为当前列填充默认值
    ...
    protected static function findByCondition($condition)
    {
        ...
        // 判断给定数组是否为关联数组,若不是关联数组，那么 $condition 只能是主键的值
        if (!ArrayHelper::isAssociative($condition)) {
            ...
            if (isset($primaryKey[0])) {
                // 当存在多个主键时，只认第一个主键属性
                $pk = $primaryKey[0];
                // 若进行了联表，为避免两个表中相同字段名冲突问题（ambiguity），在字段名前拼接表名
     ...
     * 使用提供的属性值和条件更新整个表。
     *
     * 注意 updateAll(), updateAllCounters() 和deleteAll()都不会触发事件
     *
    ...
    {
        $n = 0;
        // 遍历计数器 ['age' => 1, 'weight' => 2]
        foreach ($counters as $name => $value) {
            // 创建数据库表达式对象。预处理数据，先用占位符占位，后绑定值，防止sql注入
    ...
    {
        // 创建 ActiveQuery 实例， 并将 调用该方法的类名传入 ActiveQuery 的构造方法
        /** @var ActiveQuery::__construct($modelClass, $config = []) */
     ...
     * 默认该方法通过调用[[Inflector::camel2id()]]方法加上前缀[[Connection::tablePrefix]]返回类名作为表名称。
     * 例如 [[Connection::tablePrefix]] 是tbl_，Customer 变成tbl_customer, OrderItem变成tbl_order_item。
     * 当表名跟默认的不一致的时候，你也可以重写此方法。
     *
    ...
    public static function tableName()
    {
        /**
         * eg:
         * get_called_class() : \common\model\UserDetail
         * StringHelper::basename('\common\model\UserDetail') : UserDetail
         * Inflector::camel2id('UserDetail', '_') : user_detail
         *
         * {{%user_detail}}
         */
     ...
     * 支持的数据库操作是：[[OP_INSERT]], [[OP_UPDATE]] 和 [[OP_DELETE]],分别对应[[insert()]], [[update()]] 和 [[delete()]] 方法
     *
     ...
     * 在一些场景中，为了确保数据一致性，你需要把一些或全部操作放入事务。
     * 你可以通过重写此方法，并返回需要事务性处理的操作，例如：
     *
     * ```php
     * return [
    ...
    {
        // 这里调用了 isTransactional()，判断当前场景下，删除操作是否需要事务支持
     ...
     * 判断当前场景下，当前操作类型，是否已经在 transactions() 中声明为需要事务支持。
     *
