...
class Request extends \yii\base\Request
{
     ...
     * When CSRF validation is enabled, forms submitted to an Yii Web application must be originated from the same application.
     * If not, a 400 HTTP exception will be raised.
     *
     ...
     * Also, to use this feature, forms submitted via POST method must contain a hidden input whose name is specified by [[csrfParam]].
     ...
     * `yii.getCsrfToken()`, respectively.
     * The [[\yii\web\YiiAsset]] asset must be registered.
     ...
     * @var array the configuration for creating the CSRF [[Cookie|cookie]].
     * This property is used only when both [[enableCsrfValidation]] and [[enableCsrfCookie]] are true.
     *
     ...
     * in session under the name of [[csrfParam]]. Note that while storing CSRF tokens in session increases security,
     * it requires starting a session for every page, which will degrade your site performance.
     *
     * 是否使用cookie来存储CSRF令牌。
     ...
     * @var string the name of the POST parameter that is used to indicate if a request is a PUT, PATCH or DELETE request tunneled through POST. Defaults to '_method'.
     * @see getMethod()
     * @see getBodyParams()
     *
     */
    ...
    public function resolve()
    {
        ...
        if ($result !== false) {
            ...
            if ($this->_queryParams === null) {
                 ...
                 */
                $_GET = $params + $_GET; // preserve numeric keys 保护数字键名不被改变，键名相同时，$params 会覆盖 $_GET中的参数
            } else {
    ...
    public function getAcceptableContentTypes()
    {
        if ($this->_contentTypes === null) {
            if (isset($_SERVER['HTTP_ACCEPT'])) {
                // eg: $_SERVER['HTTP_ACCEPT'] = 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'
     ...
     * This token is generated in a way to prevent [BREACH attacks](http://breachattack.com/).
     * It may be passed along via a hidden field of an HTML form or an HTTP header value to support CSRF validation.
     *
     * 是否重新生成CSRF令牌。
     * 当此参数为true时，每次调用该方法时，将生成一个新的CSRF令牌，并持久保存(在会话或cookie中)。
     * @param boolean $regenerate whether to regenerate CSRF token.
     * When this parameter is true, each time this method is called, a new CSRF token will be generated and persisted (in session or cookie).
    ...
    public function getCsrfToken($regenerate = false)
    {
        ...
        if ($this->_csrfToken === null || $regenerate) {
            ...
            /**
             * str_repeat($chars, 5) : 将字符串 $chars 重复5次。
             * str_shuffle() : 随机地打乱字符串中的所有字符
             * substr($str, 0, 8)： 截取字符串前 8 个字符
             */
            $mask = substr(str_shuffle(str_repeat($chars, 5)), 0, static::CSRF_MASK_LENGTH);
            // The + sign may be decoded as blank space later, which will fail the validation
            /**
             * $this->xorTokens($token, $mask) 返回两个字符串的XOR(异或)结果.
             * 将$mask拼接在前面
             * 进行base64编码
             * 将编码后字符串中的 + 都替换成 .
             *
     ...
     * @return string the CSRF token loaded from cookie or session.
     * Null is returned if the cookie or session does not have CSRF token.
     */
    ...
    /**
     * 返回两个字符串的XOR(异或)结果.
     * 如果两个字符串长度不同，那么短字符串就会被加到与长字符串相同长度。
    ...
    private function xorTokens($token1, $token2)
    {
         ...
         */
        if ($n1 > $n2) {
            // 若 $token2 较短，则重复token2字符，直到与 $token1 长度相同
            $token2 = str_pad($token2, $n1, $token2);
        } elseif ($n1 < $n2) {
            // 若$token1 较短，则当$token1 长度为0时，用空格填充，否则重复$token1，直到与token2长度相同
            $token1 = str_pad($token1, $n2, $n1 === 0 ? ' ' : $token1);
        }

        // 将 $token1, $token2 进行异或运算
