 ...
 * To do so, override [[useCustomStorage]] so that it returns true, and override these methods with the actual logic about using custom storage:
 ...
 * After that, it will be deleted automatically. Flash messages are particularly useful for displaying confirmation messages.
 * To use flash messages, simply call methods such as [[setFlash()]], [[getFlash()]].
 *
 * Flash消息(key => message or key => [message1, message2])。这个属性是只读的。
 * @property array $allFlashes Flash messages (key => message or key => [message1, message2]). This property is read-only.
 * 会话cookie参数。这个属性是只读的
 ...
 * @property string $flash The key identifying the flash message. Note that flash messages and normal session variables share the same name space.
 * If you have a normal session variable using the same name, its value will be overwritten by this method. This property is write-only.
 ...
 * @property SessionIterator $iterator An iterator for traversing the session variables. This property is read-only.
 * 当前会话名
...
class Session extends Component implements \IteratorAggregate, \ArrayAccess, \Countable
{
     ...
     * If you send session ID via other ways, you may need to override this method or call [[setHasSessionId()]] to explicitly set whether the session ID is sent.
     ...
     * cookie参数，有效键包括:`lifetime`, `path`, `domain`, `secure` and `httponly`.
     ...
     * A flash message will be automatically deleted after it is accessed in a request and the deletion will happen in the next request.
     ...
     * @param boolean $removeAfterAccess whether the flash message should be automatically removed only if it is accessed.
     * If false, the flash message will be automatically removed after the next request, regardless if it is accessed or not.
     * If true (default value), the flash message will remain until after it is accessed.
     * @see getFlash()
     * @see addFlash()
     * @see removeFlash()
     */
