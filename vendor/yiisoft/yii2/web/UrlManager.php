 ...
 * @property string $hostInfo The host info (e.g. "http://www.example.com") that is used by [[createAbsoluteUrl()]] to prepend to created URLs.
 *
 * 由[[createUrl()]]所使用的入口脚本URL，用于创建URL
...
class UrlManager extends Component
{
     ...
     * Instead of putting all parameters in the query string part of a URL, pretty URLs allow using path info to represent some of the parameters and can thus produce more user-friendly URLs,
     * such as "/news/Yii-is-released", instead of "/index.php?r=news%2Fview&id=100".
     *
     ...
     * A special shortcut format can be used if a rule only specifies [[UrlRule::pattern|pattern]] and [[UrlRule::route|route]]: `'pattern' => 'route'`.
     * That is, instead of using a configuration array, one can use the key to represent the pattern and the value the corresponding route.
     ...
     *
     * For RESTful routing the mentioned shortcut format also allows you to specify the [[UrlRule::verb|HTTP verb]] that the rule should apply for.
     * You can do that by prepending it to the pattern, separated by space.
     * For example, `'PUT post/<id:\d+>' => 'post/update'`.
     * You may specify multiple verbs by separating them with comma like this: `'POST,PUT post/index' => 'post/create'`.
     * The supported verbs in the shortcut format are: GET, HEAD, POST, PUT, PATCH and DELETE.
     * Note that [[UrlRule::mode|mode]] will be set to PARSING_ONLY when specifying verb in this way so you normally would not specify a verb for normal GET request.
     *
     ...
     * make sure you populate the array with rule objects instead of rule configurations.
     *
     * 用于为urlManager声明路由规则。
    ...
    public function init()
    {
        ...
        if ($this->cache instanceof Cache) {
            ...
            // cache中是一个数组, 0号元素用于缓存创建好的路由规则，1号元素用于保存HASH值。
            // 这个判断用于确认是否有缓存、且缓存仍有效。
    ...
    public function createUrl($params)
    {
        ...
        if ($this->enablePrettyUrl) {
            ...
            if ($url === false) {
                ...
                foreach ($this->rules as $rule) {
                    /* @var $rule UrlRule */
                    // 默认GET参数不为空 并且 该规则不是仅用于URL解析，不缓存结果
