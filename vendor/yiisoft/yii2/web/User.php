...
class User extends Component
{
     ...
     *
     * @param boolean $checkAcceptHeader whether to check if the request accepts HTML responses. Defaults to `true`.
     * When this is true and the request does not accept HTML responses the current URL will not be SET as the return URL.
     * Also instead of redirecting the user an ForbiddenHttpException is thrown. This parameter is available since version 2.0.8.
    ...
    public function loginRequired($checkAjax = true, $checkAcceptHeader = true)
    {
        ...
        // 先存储 returnUrl
        if ($this->enableSession // 启用session
            && $request->getIsGet() // 是GET请求
            && (!$checkAjax || !$request->getIsAjax()) // 不是Ajax请求
            && $canRedirect
        ) {
