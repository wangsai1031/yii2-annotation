...
class Controller extends Component implements ViewContextInterface
{
    ...
    public function runAction($id, $params = [])
    {
        ...
        foreach ($this->getModules() as $module) {
            // 触发$module的 beforeAction 事件
            if ($module->beforeAction($action)) {
                // 将 $module 插入到 $modules 数组前
                array_unshift($modules, $module);
            } else {
                // 若 beforeAction 反回了 false,则将$runAction置为false,即后续不再执行 action 方法
                $runAction = false;
                break;
            }
        }
        ...
        if ($runAction && $this->beforeAction($action)) {
            ...
            // 触发控制器的 afterAction 事件
            $result = $this->afterAction($action, $result);

            // call afterAction on modules
            // 依次调用每个模块下的afterAction事件
    ...
    public function createAction($id)
    {
        ...
        // 匹配 $id 是否只包含 小写字母，数字，'-', '_'，并且中间没有两个连续的'--',且两侧都没有 '-'。
             ...
             * 格式化 action 方法名：
             * eg: user-view
             * explode('-', $id)    => ['user', 'view']
             * implode(' ', $str)   => user view
             * ucwords($str)        => User View
             * str_replace(' ', '', $str) => UserView
             */
            $methodName = 'action' . str_replace(' ', '', ucwords(implode(' ', explode('-', $id))));
            // 判断该action方法是否存在
            if (method_exists($this, $methodName)) {
                // ReflectionMethod 类报告一个方法的有关信息。
                $method = new \ReflectionMethod($this, $methodName);
                // $methodName 方法是 public 方法，且方法名正确
    ...
    public function getModules()
    {
        $modules = [$this->module];
        $module = $this->module;
        // 多层 $module 嵌套时，循环将外层 $module 添加到 $modules 数组开头
        while ($module->module !== null) {
            // 将 $module->module 添加到 $modules 数组开头
