 ...
 *
 * @see ArrayAccess 提供作为数组访问对象的接口
 * @see IteratorAggregate 创建外部迭代器的接口
 *
...
class Model extends Component implements StaticInstanceInterface, IteratorAggregate, ArrayAccess, Arrayable
{
     ...
     * 被验证的属性名称列表。如果该参数为空，意味着使用验证规则的属性都会被验证。
     *
     * @param bool $clearErrors whether to call [[clearErrors()]] before performing validation
     * 验证执行以前是否调用clearErrors方法
     *
    ...
    public function validate($attributeNames = null, $clearErrors = true)
    {
        ...
        // 未设置要验证的属性，则获取在当前场景中接受验证的所有属性名称。
    ...
    {
        // \ArrayObject 这个类允许对象作为数组来工作
        $validators = new ArrayObject();
        // 遍历rules()
        foreach ($this->rules() as $rule) {
            // 若 $rule 是 Validator 的实例，则直接将该$rule附加到 $validators 数组末尾
            ...
            // 是数组，则至少有两个元素
                 ...
                 *
                 * $rule[0]：需要该验证器验证的属性
                 * $rule[1]：可以是一个内置的验证器名称，某个模型类的方法名，一个匿名行数，或者一个验证器类名。
                 * array_slice($rule, 2) ： $rule 除去前两个元素其余的元素
                 */
                $validator = Validator::createValidator($rule[1], $this, (array) $rule[0], array_slice($rule, 2));
                // 将验证器对象附加到 $validators 数组末尾
    ...
    public function getAttributeLabel($attribute)
    {
        ...
        // 如果存在该标签，则直接返回，不存在则自己生成(将驼峰法字符串转换成空格分隔的单词)
    ...
    public function activeAttributes()
    {
        ...
        // 遍历，去掉非安全属性(eg:['username', 'password', '!secret']中的 '!secret') 前的叹号
    ...
    /**
     * 当没有指定特定字段时，返回默认由toArray()默认返回的字段列表
