 ...
 * 属性 \yii\caching\CacheInterface $cache 缓存应用组件。如果该组件没有开启，将会返回null，该属性只读
 * 
...
abstract class Application extends Module
{
     ...
     * 属性 动作，请求的动作。如果是null，则意味着请求不能被解析为一个动作。
     */
    ...
    {
        // 将\yii\base\Application里所有的公共方法都交给了，Yii::$app,其实Yii大部分信息都在Yii::$app变量中
        // 在很多地方我们会调用Yii::$app，就是调用了Application相对应的实体，也就是整个应用程序。
        Yii::$app = $this;
        ...
        //加载配置文件的框架信息 如：设置别名，设置框架路径等等 最为重要的是给加载默认组件
        $this->preInit($config);

        //加载配置文件中的异常组件
        $this->registerErrorHandler($config);

        // 将配置文件中的所有信息赋值给Object，也就是Yii::$app->配置文件参数可以直接调用配置文件的内容
        // 如：
        //  Yii::$app->vendorPath//输出框架路径
        //  Yii::$app->components['redis']//输出redis配置信息
    ...
    {
        /**
         * 当使用Composer安装扩展后，会向 @vendor/yiisoft/extensions.php 写入信息，其中就包含相应的别名。
         * 只不过这些别名通常都是二级别名。
         * 然后，在 yii\base\Application::bootstrap() 中，将这些扩展的别名进行注册。
         * */
        if ($this->extensions === null) {
            // 通过别名获取 extensions.php 文件
            $file = Yii::getAlias('@vendor/yiisoft/extensions.php');
            // 若存在 extensions.php 文件， 则使用 include函数获取文件返回内容，并赋值给 $this->extensions
            $this->extensions = is_file($file) ? include $file : [];
        }
        // 遍历 $this->extensions
        foreach ($this->extensions as $extension) {
            if (!empty($extension['alias'])) {
                // 若存在 'alias' 元素，则遍历设置全局 别名
                foreach ($extension['alias'] as $name => $path) {
                    Yii::setAlias($name, $path);
                }
            }
            if (isset($extension['bootstrap'])) {
                // 若存在 'bootstrap' 元素，根据配置创建对象
                $component = Yii::createObject($extension['bootstrap']);
                // 若 $component 继承自引导接口
                if ($component instanceof BootstrapInterface) {
                    Yii::debug('Bootstrap with ' . get_class($component) . '::bootstrap()', __METHOD__);
                    // 初始化扩展，执行extensions中的引导组件
        ...
        foreach ($this->bootstrap as $mixed) {
            ...
            if ($mixed instanceof \Closure) {
                ...
                if ($this->has($mixed)) {
                    ...
                    // 若该组件 注册为 module,则直接获取该组件
                    $component = $this->getModule($mixed);
                } elseif (strpos($mixed, '\\') === false) {
                    // 若以上都不是，且 $class 中不存在 '\',则抛异常
            ...
            // 若 $component 实现了 BootstrapInterface接口
            if ($component instanceof BootstrapInterface) {
                Yii::debug('Bootstrap with ' . get_class($component) . '::bootstrap()', __METHOD__);
                // 调用该组件的引导方法
                $component->bootstrap($this);
    ...
    public function run()
    {
        try {
            ...
            // 加载控制器的地方，获取用户请求，并进行处理，处理的过程也是产生响应内容的过程
