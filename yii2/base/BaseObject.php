...
class BaseObject implements Configurable
{
    ...
    public function __construct($config = [])
    {
        if (!empty($config)) {
            ...
            //由于Object是大部分类的基类，
            //实际上也就是交给了yii\web\Application
            //您可以Yii::$app->配置参数来访问配置文件中的内容
     ...
     * @see __set()
     *
     * __set() 魔术方法，当调用类不存在的属性时，调用该方法，并将改属性名作为参数传给该方法
     */
    public function __get($name)
    {
        // getter函数的函数名
        // 由于php中方法名称不区分大小写，所以getproperty() 等价于 getProperty()
        $getter = 'get' . $name;
        if (method_exists($this, $getter)) {
            // 如果getter函数存在，则调用了getter函数
            return $this->$getter();
        } elseif (method_exists($this, 'set' . $name)) {
            // 如果只有setter没有getter 则为只写属性
    ...
    /**
     * __isset() 当 用isset() 判断对象不可见的属性时(protected/private/不存在的属性) 会引发 __isset()来执行
     *
     * 用于测试属性值是否不为 null ，在 isset($object->property) 返回false 时被自动调用。
     *
     * 判断是否存在该属性相应的getter。
     *
     ...
     * 用于将属性值设为 null ，在 unset($object->property) 而 $object->property 不存在时被自动调用。
     *
     * 调用该属性相应的setter方法。
     *
    ...
    /**
     * 用于测试是否有某个属性。即，定义了getter或setter。
     * 如果 hasProperty() 的参数 $checkVars = true （默认为true）， 那么只要具有同名的成员变量也认为具有该属性，如前面提到的 public $title 。
     *
    ...
    /**
     * 测试一个属性是否可读，参数 $checkVars 的意义同上。只要定义了getter，属性即可读。
    ...
    /**
     * 测试一个属性是否可写，参数 $checkVars 的意义同上。只要定义了setter，属性即可写。
