...
class Module extends ServiceLocator
{
     ...
     * 比如说默认的控制器SiteController吧。
     * 在里面写一个init方法，当你访问site控制器下任意的$route路径,
    ...
    public function getBasePath()
    {
        if ($this->_basePath === null) {
            /**
             * 若 $this->_basePath 为空，则：
             * eg:
             * $class->getFileName()    => '/vagrant/nest/vendor/yiisoft/yii2/web/Application.php'
             * dirname($dir)            => '/vagrant/nest/vendor/yiisoft/yii2/web'
             */
    ...
    public function setBasePath($path)
    {
        ...
        // 若$path 以 'phar://' 开头，则直接使用该路径，否则通过 realpath() 函数返回规范化的绝对路径名
        $p = strncmp($path, 'phar://', 7) === 0 ? $path : realpath($path);
        if ($p !== false && is_dir($p)) {
            // 若路径符合规范，则赋值给 $this->_basePath
            $this->_basePath = $p;
        } else {
    ...
    public function runAction($route, $params = [])
    {
        ...
        if (is_array($parts)) {
            ...
            //获得$actionId和$controller
            list($controller, $actionID) = $parts;
            $oldController = Yii::$app->controller;
            Yii::$app->controller = $controller;
            //运行使用控制器加载 action方法
            $result = $controller->runAction($actionID, $params);
            //将对象交给Yii::$app->controller 这里面起的作用应该是运行控制器，最后释放控制器的对象变量
     ...
     * 2. 如果路由的第一部分是在[[modules]]中声明的有效的模块ID，根据路由剩余的部分调用模块的`createController()`方法
     *
     ...
     * 4. 给定的路由是`abc/def/xyz`格式，就会在[[controllerNamespace|controller namespace]]控制器命名空间下，尝试`abc\DefController`或者`abc\def\XyzController`
     *
    ...
    {
        if ($route === '') {
            // 若$route为空字符串，则使用默认路由
            $route = $this->defaultRoute;
        }

        // double slashes or leading/ending slashes may cause substr problem
        // 去除路由两头的斜杠，双斜杠或者开头/结尾的斜杠可能会导致子字符串问题
        $route = trim($route, '/');
        // 判断是否有两个连续的斜杠
        ...
        if (strpos($route, '/') !== false) {
            // 若路由中存在斜杠，第一个斜杠前的赋值给 $id ，后的赋值给 $route
        ...
        // 优先考虑模块和控制器映射表controllerMap
        if (isset($this->controllerMap[$id])) {
            // 若控制器映射表中存在 指定映射关系，则根据该映射创建控制器实例
    ...
    public function createControllerByID($id)
    {
         ...
         * 格式化$className
         * eg: user-detail
         * str_replace('-', ' ', $className)    =>  user detail
         * ucwords($str)                        =>  User Detail
         * str_replace(' ', '', $str)           =>  UserDetail
         */
        ...
        // 若 $className 中依然存在 '-' 或者不存在 $className 这个类，则返回 null
         ...
         *
         * 判断 $className 是否是 'yii\base\Controller' 的子类
         */
        if (is_subclass_of($className, 'yii\base\Controller')) {
            // 创建 controller 对象， 并制定 id 和 module
            $controller = Yii::createObject($className, [$id, $this]);
            // 判断 $controller 的类名 是否与 $className 完全相同，返回 controller对象
    ...
    {
        // % 为定界符，匹配$className 是否以小写字母开头，且只包含小写字母，数字，'-','_'
        if (!preg_match('%^[a-z][a-z0-9\\-_]*$%', $className)) {
            return true;
        }
        // $prefix要么为空字符串，要么 且只包含小写字母，数字,'_'
